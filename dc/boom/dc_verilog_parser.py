...
class DcParser:
    ...
    def parse_port(
        ...
        if 'alu_DP_OP' in mcomp: print(portname,argname)
        #if 'add_x_94' in mcomp: print(portname, argname)
    ...
    def parse(self, fname, label_region=False):
        ...
        for item in top_module.items:
            ...
            if 'alu_DP_OP' in mcomp:
              print('\n',mcell,mname,len(ports))
...
def main():
    ...
    for v in os.listdir(folder):
        ...
        parser = DcParser("BoomCore", ["alu_DP_OP"], "xor")
