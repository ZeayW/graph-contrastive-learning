...
class DcParser:
        ...
        for kw in self.adder_keywords:
            if kw in mcomp and mcomp != argcomp:
                ...
                if self.is_output_port(portname):
                    ...
                    if 'div_DP_OP' in mcomp and position[0] in ('I1','I2'):
                        #print(argname)
                        return port_info
    ...
    def parse_nohier(self, fname, target_cells,label_region=False):
        """ parse dc generated verilog """
        adder_cells = set()
...
def main():
    ...
    for vfile_pair in vfile_pairs:
        ...
        # parser = DcParser("BoomCore", ["alu_DP_OP", "add_x"])
        parser = DcParser("Rocket", hier_keywords=["MulDiv_DP_OP","ALU_DP_OP", "add","inc"],adder_keywords=['add_x','alu_DP_OP','div_DP_OP'],hadd_type="xor")
        parser.parse_report(hier_report)
        exit()
        target_cells = parser.parse_hier(hier_vf)

        nodes, edges = parser.parse_nohier(vf, target_cells=target_cells,label_region=True)
