...
class DcParser:
    ...
    def parse_port(
        self, mcomp: str, port: pyverilog.vparser.parser.Portlist,inputs:list
    ) -> PortInfo:
    ...
    def parse(self, fname, label_region=False):
        ...
        for item in top_module.items:
           ...
           # exit()
            for p in ports:
                port_info = self.parse_port(mcomp, p,inputs)
...
def main():
    ...
    for v in os.listdir(folder):
        ...
        parser = DcParser("mips", ["add_x"], "xor")
