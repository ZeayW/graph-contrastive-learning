...
class PortInfo:
    ptype: str                  # port type: fanin / fanout
    portname: str
    argname: str
    argcomp: str
    is_adder_input: bool
...
class DcParser:
    ...
    # mcomp: port归属的cell所在的module e.g add_x
    def parse_port(
        self, mcomp: str, port: pyverilog.vparser.parser.Portlist,
    ) -> PortInfo:
        # argname为port在网表中的名称，如 add_x_5_n3 / n3732
        ...
        # 输出端口
            ...
            for kw in self.adder_keywords:
                # mcomp==argcomp是说明port在component内部，不是IO
    ...
    def parse(self, fname, label_region=False):
        ...
        for item in top_module.items:
            ...
            mcell = instance.module  # cell name e.g. AND2X1
            mname = instance.name    # cell所属的module
            ports = instance.portlist # cell的IO，e.g., add有 A,B,S,CI,CO
            # mtype ： 将mcell截取到X之前
            ...
            # cell的fanin实际上是前面的gate的输出，故而边就是 fanin-->fanout
            for fi in fanins:
                for fo in fanouts:
                    edges.append(
                        (
