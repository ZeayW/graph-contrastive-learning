...
class DcParser:
    ...
    def parse_port(
        self, mcomp: str,target_cells: list,port: pyverilog.vparser.parser.Portlist,index01:list
    ) -> PortInfo:
        portname, argname = port.portname, port.argname
            ...
            if kw in mcomp and mcomp != argcomp:
                ...
                for mport in module_ports.keys():
                    ...
                    for i, arg in enumerate(mport_args):
                        if arg[arg.find('_') + 1:].lower() in argname.lower():
    ...
    def parse_hier(self, fname):
        ...
        for module in ast.description.definitions:
            ...
            for item in module.items:
                ...
                for key_word in self.hier_keywords:
                    if key_word in mcell:
                        print(mname)
                    ...
                    if args_to_update.get(mcell, None) is not None:
                        ...
                        for (child_cell_name, child_instance_name,
                            ...
                            for cell_info in target_cells[mcell]:
                                if cell_info.cell_name == child_cell_name and cell_info.instance_name == child_instance_name:
        ...
        #     print(cell.cell_name, cell.instance_name)
        # exit()

        return target_cells

    def parse(self, fname, target_cells,label_region=False):
        """ parse dc generated verilog """
        adder_cells = set()
        ...
        for item in top_module.items:
           ...
           # exit()
            for p in ports:
                port_info = self.parse_port(mcomp, target_cells,p,index01)
            ...
            for fo in fanouts:
                if mfunc == "HADD":
                    ...
                    ntype = mfunc
                nodes.append((fo.argname, {"type": ntype,"position":fo.position}))
            for fi in fanins:
                for fo in fanouts:
                    edges.append(
                        (
        ...
        if label_region:
            ...
            for n in nodes:
                ...
                n[1]['position'] = n[1].get('position',0)
...
def main():
    ...
    for v in os.listdir(folder):
        if not v.endswith('v') or '10' in v:
            continue
    ...
    for vfile_pair in vfile_pairs:
        ...
        parser = DcParser("BoomCore", hier_keywords=["ALU_DP_OP", "add","inc"],adder_keywords=['add_x','alu_DP_OP'],hadd_type="xor")
        target_cells = parser.parse_hier(hier_vf)

        nodes, edges = parser.parse(vf, target_cells=target_cells,label_region=True)
