...
def predict_single_dag(device,model,g,Loss,label,options,beta):
    start = time()
    #print("beta:",beta)
    if label == 'input':
        label_name = 'label_i'
   ...
   # return predicts,acc,recall,precision,F1_score
    return predicts,runtime

def predict_single_udg(device,model,g,Loss,label,options,beta):
...
def predict(options):
    ...
    data_path = "testdata"
    print('testcase\tinput\toutput\toverall')
    runtimes = []
        ...
        if os.path.isdir(model_path+model):
            ...
            for file in os.listdir(data_path):
                ...
                for i in range(1,6):
                    ...
                    predict_i,time_i = predict_single(device, model_in, g, Loss,
                                                                                         'input', param_in,
                                                                                         options.beta)
                    #input_scores.append([acc_i, recall_i, precision_i, F1_score_i])
                    predict_o ,time_o= predict_single(device, model_out, g, Loss,
                                                                                         'output', param_out,
                                                                                         options.beta)
                    ...
                    runtime_i += time_i
                    runtime_o += time_o
                    save_path = os.path.join(model_path,model+'/predict/')
                    if os.path.exists(save_path) is False:
                        os.mkdir(save_path)
                    with open(os.path.join(save_path , '{}.pkl'.format(i)), 'wb') as f:
                        pickle.dump((predict_i, predict_o), f)

                print(casename.split('_')[0],'\t\t',round(runtime_i/5,2),'\t',round(runtime_o/6,2),'\t',round(runtime_i/5+runtime_o/5,2))
