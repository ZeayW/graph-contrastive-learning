...
def validate(val_graphs,device,model,mlp,combine,Loss,alpha,beta,options):
    ...
    with th.no_grad():
        ...
        for idx, (label, graph, POs, depth) in enumerate(val_graphs):
            depth = min(options.depth_thred,depth)
        ...
        error_mask = predict_labels != labels

        #print('val ground-truth labels:', labels[error_mask])
        #print('val predict labels:', predict_labels[error_mask])
...
def train(options):
    ...
    if freeze:
        ...
        optim = th.optim.Adam(
            ...
            itertools.chain(model.parameters(),mlp.parameters()),
            #mlp.parameters(),
            options.learning_rate, weight_decay=options.weight_decay
        )
