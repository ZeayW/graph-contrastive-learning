...
from dataset_sub import Dataset_sub
...
def preprocess(data_path,device,options):
    ...
    train_data_file = os.path.join(data_path, 'adder2.pkl')
    ...
    if os.path.exists(train_data_file) is False:
        datapaths = ["../dc/sub/implementation/adder32/"]
        th.multiprocessing.set_sharing_strategy('file_system')
        #print(dataset_not_edge.Dataset_n)
        dataset = Dataset_sub("adder",options.width,datapaths,label2id)
        #print(g.ndata)
        print(dataset.batch_graph.ndata)
        #print(g.edata['r'])
        with open(train_data_file,'wb') as f:
            pickle.dump((dataset.batch_graph,dataset.PIs,dataset.POs,dataset.depth,dataset.width,dataset.num_aug),f)
...
def train(options):
    ...
    train_data_file = os.path.join(data_path,'adder.pkl')
    ...
    with open(train_data_file,'rb') as f:
        train_g,PIs,POs,depth,width,num_aug = pickle.load(f)

    with open(neg_data_file, 'rb') as f:
        neg_g = pickle.load(f)
        neg_nodes = get_neg_nodes(neg_g,'label_o')
        print('num_rocket:{}, num_neg:{}'.format(neg_g.number_of_nodes(),len(neg_nodes)))
    ...
    pos_pairs = get_pos_pairs(width,num_aug)
    print(train_blocks)
    # print(pos_pairs)
    print(po_depths)
